...
CommandShortcutEventHandler _upInTableCellHandler = (editorState) {
  ...
  if (_hasSelectionAndTableCell(inTableNodes, selection)) {
    try {
      final previousNode = _getPreviousNode(inTableNodes, 0, 1);
      if (_nodeHasTextChild(previousNode)) {
        final target = previousNode!.childAtIndexOrNull(0)!;
...
Node? _getNextNode(Iterable<Node> nodes, int colDiff, int rowDiff) {
  ...
  if (isLeavingTableVertically(nextRow, numRows)) {
    final next = getSibling(table, false);
...
Node? _getPreviousNode(Iterable<Node> nodes, int colDiff, int rowDiff) {
  ...
  if (isLeavingTableVertically(prevRow, numRows)) {
    final next = getSibling(table, true);
    if (next != null) throw OutOfBoundsException(next);
  }
